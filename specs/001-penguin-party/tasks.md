...
| **테스트(test-writer, Opus 4.6)** | `tests/**` — 스토리별 하위 폴더로 분리(`tests/unit/game/`는 F, `tests/unit/roster/`는 US1, …) |
...
- [ ] T009 (test-writer) 프로토콜·순위 단위 테스트 `tests/unit/p2p/protocol.test.ts` — stale seq/raceId 폐기, 동점 규칙(distance ↓ → distanceReachedAt ↑ → 입장순), 스냅샷 직렬화 왕복
...
- [ ] T016 (test-writer) [US1] 로스터 규칙 단위 테스트 `tests/unit/roster/roster.test.ts` — FR-003·004·020·026: 빈 닉네임 거부, 15정원(재접속 미산입), 중복 접미사 " (2)", 로비 외 phase 입장 거부, playerId 재접속 승계
...
- [ ] T020 (test-writer) [US2] 레이스 흐름 통합 테스트 `tests/unit/race/race-flow.test.ts` — FR-006·007·012·019: race-start 브로드캐스트→로컬 타이머, 끊김 시 마지막 state로 확정, 이전 raceId 스냅샷 폐기, race-end 집계
...
- [ ] T024 (test-writer) [US3] 순위·재경기 단위 테스트 `tests/unit/result/ranking.test.ts` — FR-015·017·021: 내림차순 rank, 동점 3단계 타이브레이크, return-lobby 후 raceId 증가·기록 초기화·멤버 유지
